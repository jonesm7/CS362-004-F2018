...
void checkCardPilesDontChange(gameState* state, int checkKingdomCards, int checkVictoryCards, int checkTreasureCards) {
  struct gameState* initialState = malloc(sizeof(struct gameState));
  int playerIndex = 0;
  int randomSeed = 2;
  int numPlayers = 2;
  int i;
  initializeGame(numPlayers, kingdomCards, randomSeed, initialState);
  if (checkKingdomCards) {
    for (i = 0; i < 10; i++) {
      int card = kingdomCards[i];
...
int main() {
  ...
  checkIntEquals(baron, state1->hand[handCount-1], "cardEffect(smithy): hand[-1]");
  checkIntEquals(great_hall, state1->hand[handCount-2], "cardEffect(smithy): hand[-2]");
  checkIntEquals(minion, state1->hand[handCount-3], "cardEffect(smithy): hand[-3]");
  ...
  checkIntEquals(5, state1->handCount[1], "cardEffect(smithy): other player's hand");
